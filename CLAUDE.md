...
- **Spatial audio is set via `IPropertyStore` write.** `SpatialAudioController.SetMode` opens the endpoint's property store with `STGM_READWRITE` and writes `PKEY_AudioEndpoint_Spatial_Audio_Format` (`{F19F064D-082C-4E27-BC73-6882A1BB8E4C},5`) as a DWORD. The mode→DWORD mapping lives in `SpatialAudioFormatRegistry` (Stereo=0, Sonic=1, Atmos=2, DTS=3) — those values mirror what Windows Sound Settings writes. Most Windows 11 endpoints accept the write without admin; locked-down/enterprise endpoints return `E_ACCESSDENIED`, which `ProfileApplier.TryStep` records as a step error (no crash). Atmos/DTS DWORDs are accepted even when Dolby Access / DTS Sound Unbound aren't installed — the audio engine silently falls back to stereo, so don't try to "validate" availability before writing.
- **APO config split between two files.** `ApoConfigWriter` owns `<APO config dir>/audioswitch.txt` (rewritten on every Write, contains only generated `Device:` blocks) and only adds an idempotent `Include: audioswitch.txt` line to APO's `config.txt`. We never modify the rest of `config.txt`, so user-edited filters survive AudioSwitch updates and uninstall is "delete one file + strip one line." This split is load-bearing — don't be tempted to write everything into config.txt directly.
- **Bundled APO installer.** `src/AudioSwitch.App/Assets/Tools/EqualizerAPO64.exe` is the official installer downloaded from SourceForge (SHA-256 + source URL in the same folder's `NOTICES.md`). It's GPLv2; redistribution requires keeping the NOTICES file alongside any binary distribution. The csproj copies it to the output dir via `<None CopyToOutputDirectory>`, and `.gitignore` has an explicit allow-rule because `*.exe` is otherwise excluded. `ApoInstallHelper.LaunchInstaller()` uses `UseShellExecute = true` so UAC elevation happens via the installer's own manifest — don't change to `false`, that would suppress the elevation prompt and the install would silently fail.
